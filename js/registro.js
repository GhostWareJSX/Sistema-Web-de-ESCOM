...
const reglas = {
  ...
  telefono: /^\d{10}$/, ///^(\+52)?\d{10}$/
  correo: /^[a-zA-Z0-9](?!.*[.]{2})[a-zA-Z0-9!#$%&'*+\-/=?^_`{|}~.]*[a-zA-Z0-9]@alumno\.ipn\.mx$/,
  contrasena: /^(?=.*[A-Z])(?=.*\d)(?=.*[^a-zA-Z0-9\s])\S{6,}$/,
};
...
function validarContrasena() {
  ...
  if (!esValido) {
    ...
    if (/\s/.test(valor)) mensajes.push("no debe contener espacios");
...
function validarFormulario() {
  ...
  errCorreo.textContent = correoValido
    ? ""
    : "El correo debe tener el formato usuario@alumno.ipn.mx";
...
// Handler para confirmar el registro: muestra alerta y redirige
const btnConfirmar = document.getElementById("btnConfirmar");
if (btnConfirmar) {
  btnConfirmar.addEventListener("click", function () {
    const alertCont = document.getElementById("alertRegistro");
    if (alertCont) {
      alertCont.innerHTML = `
        <div class="alert alert-success alert-dismissible fade show" role="alert">
          <strong>Registro exitoso.</strong> Serás redirigido a la página de inicio.
          <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Cerrar"></button>
        </div>`;
      alertCont.scrollIntoView({ behavior: "smooth" });
    }
